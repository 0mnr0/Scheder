...
{
    public Task<int?> GetValueAsync(long userId, int settingId, bool isGroup, CancellationToken cancellationToken) =>
        isGroup ? Memory.Group.GetSettingAsync(userId, settingId) : Memory.User.GetSettingAsync(userId, settingId);
 
    public Task SetValueAsync(long userId, int settingId, int value, bool isGroup, CancellationToken cancellationToken) =>
        isGroup ? Memory.Group.SetSettingAsync(userId, settingId, value) : Memory.User.SetSettingAsync(userId, settingId, value);
 
    public Task<Dictionary<int, int>> GetAllValuesAsync(long userId, bool isGroup, CancellationToken cancellationToken) =>
